...
%%
filename = 'vpul2da'+string(a)+'b'+string(b)+'d'+string(d)+'amp'+string(alpha)+'.gif'; 
...
for i=1:N
  for j=1:N
    ...
    r=x^2+y^2;
    if ((r>=a2)&(r<=b2))
      U(i,j)=mod(atan2(y,x)+2*pi+rand()*.2,2*pi);
      %U(i,j)=2*pi*rand();
      M(i,j)=1;
    end
  end
...
save('vv_pul2da'+string(a)+'b'+string(b)+'d'+string(d)+'amp'+string(alpha)+'.mat','V')
