...
%%
filename = 'pul2da'+string(a)+'b'+string(b)+'d'+string(d)+'amp'+string(alpha)+'.gif'; 
...
for t=1:T
   ...
   Up(:)=1+Q.*WP;
   U(:)=U(:)+dt*Up(:);
   if mod(t,2000) == 0 %change delaytime when change mod
        h0=pcolor(q,q,mod(U,2*pi).*M);
        colormap('hsv')
        set(h0,'EdgeColor','none');
        drawnow;
...
save('v_pul2da'+string(a)+'b'+string(b)+'d'+string(d)+'amp'+string(alpha)+'.mat','V')
